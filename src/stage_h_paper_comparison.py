...
def run_stage_h_paper_comparison(n_splits=10):
    ...
    with open(flagship_json) as f:
        d4_res = json.load(f)
    best_params = d4_res["best_gbm_params"]
    ...
    registry = {
        '[Otieno 2024] mLR  (L2 C=1, MinMax)': (
            MinMaxScaler(),
            LogisticRegression(penalty='l2', C=1.0, solver='lbfgs',
                               max_iter=2000, class_weight='balanced', random_state=42)),
        '[Otieno 2024] SVM  (RBF C=10 g=0.01, MinMax)': (
            MinMaxScaler(),
            SVC(kernel='rbf', C=10.0, gamma=0.01, probability=True,
                class_weight='balanced', random_state=42)),
        '[Otieno 2024] ANN  (100-50 relu, MinMax)': (
            MinMaxScaler(),
            MLPClassifier(hidden_layer_sizes=(100, 50), activation='relu',
                          solver='adam', alpha=0.0001, max_iter=500, random_state=42)),
        '[Otieno 2024] XGBoost (n=100 d=6 lr=0.1, MinMax)': (
            MinMaxScaler(),
            GradientBoostingClassifier(n_estimators=100, max_depth=6,
                                       learning_rate=0.1, subsample=0.8, random_state=42)),
        '[Li 2024]   Nomogram LR (ElasticNet, StdScaler)': (
            StandardScaler(),
            LogisticRegression(penalty='elasticnet', l1_ratio=0.5, C=0.1,
                               solver='saga', max_iter=3000,
                               class_weight='balanced', random_state=42)),
        '[Liu 2026] XGBoost (n=100 d=4 lr=0.05, StdScaler)': (
            StandardScaler(),
            GradientBoostingClassifier(n_estimators=100, max_depth=4,
                                       learning_rate=0.05, subsample=0.8,
                                       min_samples_leaf=5, random_state=42)),
        '[Bogey 2025] RF  (n=100 d=5, StdScaler)': (
            StandardScaler(),
            RandomForestClassifier(n_estimators=100, max_depth=5,
                                   min_samples_leaf=3,
                                   class_weight='balanced', random_state=42)),
        '[Bogey 2025] GBM (n=100 d=3 lr=0.1, StdScaler)': (
            StandardScaler(),
            GradientBoostingClassifier(n_estimators=100, max_depth=3,
                                       learning_rate=0.1, random_state=42)),

        '[OUR FLAGSHIP] GBM + Feature Selection (Repeated 10-Fold CV)': Pipeline([
            ('sc', StandardScaler()),
        ...
        for label, model in registry.items():
            ...
            for tr_idx, va_idx in skf.split(X, y):
                ...
                if isinstance(model, tuple):
                    sclr_tmpl, clf_tmpl = model
                    sc = sclr_tmpl.__class__()
                    clf = clone(clf_tmpl)
                    Xtr_s = sc.fit_transform(Xtr); Xva_s = sc.transform(Xva)
                    clf.fit(Xtr_s, ytr)
                    prob = clf.predict_proba(Xva_s)[:, 1]
                else:
                    pipe = clone(model)
